...
export default function StoryLibraryView({ onOpenEntry }) {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {chapters.length === 0 ? (
            <div style={{ padding: '32px', textAlign: 'center', backgroundColor: '#FFFFFF', borderRadius: '12px', border: '1px dashed #CBD5E1', color: '#64748B' }}>
              No chapters written yet for this story.
            </div>
          ) : (
            chapters.map((ch, idx) => (
              <div
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      {showNewStoryForm && (
        <form onSubmit={handleCreateStory} style={{ backgroundColor: '#FFFFFF', padding: '16px', borderRadius: '12px', border: '1px solid #DC2626' }}>
          <input
            type="text"
            placeholder="Story Title (e.g. The Lost Kingdom)"
            value={newStoryTitle}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
            <button type="button" onClick={() => setShowNewStoryForm(false)} style={{ background: 'none', border: 'none', color: '#64748B', fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
            <button type="submit" style={{ backgroundColor: '#DC2626', color: '#FFFFFF', border: 'none', padding: '8px 16px', borderRadius: '8px', fontWeight: 700, cursor: 'pointer' }}>Create Story</button>
          </div>
        </form>
      )}
