...
export default function TaskDedicatedPageView({ 
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', paddingBottom: '50px' }}>
      
      {/* 1. TOP NAVIGATION HEADER & RECURSIVE BREADCRUMBS (EDIT/DELETE/ARCHIVE ONLY) */}
      <div className="glass-panel" style={{ padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexWrap: 'wrap' }}>
          <button 
            ...
            style={{ display: 'flex', alignItems: 'center', gap: '4px', fontWeight: 800, padding: '4px 8px', fontSize: '11px' }}
          >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
          <button 
            ...
            className="btn-secondary"
            style={{ color: currentTask.isArchived ? '#DC2626' : '#475569', borderColor: currentTask.isArchived ? '#DC2626' : '#CBD5E1', padding: '4px 8px', fontSize: '11px', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '3px' }}
          >
            <Archive size={12} color="#DC2626" /> {currentTask.isArchived ? 'Unarchive' : 'Archive'}
          </button>

          <button 
            onClick={() => onEditTask(currentTask)}
            className="btn-secondary"
            style={{ padding: '4px 8px', fontSize: '11px', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '3px' }}
          >
            <Edit3 size={12} color="#0F172A" /> Edit
          </button>

          <button 
            onClick={() => onDeleteTask(currentTask.id)}
            className="btn-secondary"
            style={{ color: '#DC2626', borderColor: '#FCA5A5', padding: '4px 8px', fontSize: '11px', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '3px' }}
          >
      ...
      {!isFeasible && (
        <div style={{
          ...
          <div style={{ width: '34px', height: '34px', borderRadius: '8px', background: '#DC2626', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <AlertTriangle size={18} color="#FFF" />
          </div>
          <div>
            <h4 style={{ fontSize: '13px', fontWeight: 900, color: '#991B1B', margin: '0 0 2px 0' }}>
              CRITICAL: Goal Unachievable on Current Schedule!
            </h4>
            <p style={{ fontSize: '11px', color: '#7F1D1D', fontWeight: 700, margin: 0 }}>
      ...
      {/* 2. EXECUTIVE METADATA DISPLAY GRID (CLEAN VISUAL PROGRESS BARS, NO LITERAL NUMBERS) */}
      <div className="glass-panel" style={{ padding: '16px', borderRadius: '14px' }}>
        ...
        <div style={{ marginBottom: '14px', borderBottom: '1px solid #E2E8F0', paddingBottom: '12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap', marginBottom: '4px' }}>
            <h2 style={{ fontSize: '18px', fontWeight: 900, color: '#0F172A', letterSpacing: '-0.02em', margin: 0 }}>
              {currentTask.title}
            </h2>

            <span style={{ fontSize: '10px', fontWeight: 800, color: '#475569', background: '#F1F5F9', border: '1px solid #CBD5E1', padding: '2px 6px', borderRadius: '5px', display: 'inline-flex', alignItems: 'center', gap: '3px' }}>
              <Folder size={10} color="#64748B" /> {currentTask.category || 'General'}
            </span>

            <span style={{ fontSize: '10px', fontWeight: 800, color: '#DC2626', background: '#FEF2F2', border: '1px solid #FECACA', padding: '2px 6px', borderRadius: '5px', display: 'inline-flex', alignItems: 'center', gap: '3px' }}>
              <Zap size={10} color="#DC2626" /> {(currentTask.priority || 'HIGH').toUpperCase()} Priority
            </span>

            <span style={{ fontSize: '10px', fontWeight: 800, color: '#2563EB', background: '#EFF6FF', border: '1px solid #BFDBFE', padding: '2px 6px', borderRadius: '5px', display: 'inline-flex', alignItems: 'center', gap: '3px' }}>
              <Target size={10} color="#2563EB" /> Mode: {trackingMode}
            </span>

            <span style={{ fontSize: '10px', fontWeight: 800, color: currentTask.isOptional ? '#D97706' : '#16A34A', background: currentTask.isOptional ? '#FEF3C7' : '#DCFCE7', border: currentTask.isOptional ? '1px solid #FDE68A' : '1px solid #BBF7D0', padding: '2px 6px', borderRadius: '5px' }}>
        ...
        {/* Executive Property Metrics Grid with Mini Progress Bars */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '8px' }}>
          
          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: '3px solid #64748B' }}>
            <span style={{ fontSize: '9px', fontWeight: 800, color: '#64748B', display: 'block', textTransform: 'uppercase' }}>Start Date</span>
            <span style={{ fontSize: '12px', fontWeight: 900, color: '#0F172A' }}>{currentTask.plannedStart || 'Not set'}</span>
          </div>

          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: '3px solid #DC2626' }}>
            <span style={{ fontSize: '9px', fontWeight: 800, color: '#64748B', display: 'block', textTransform: 'uppercase' }}>End Date Deadline</span>
            <span style={{ fontSize: '12px', fontWeight: 900, color: '#DC2626' }}>{currentTask.plannedEnd || 'Not set'}</span>
          </div>

          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: '3px solid #2563EB' }}>
            <span style={{ fontSize: '9px', fontWeight: 800, color: '#64748B', display: 'block', textTransform: 'uppercase' }}>Window Duration</span>
            <span style={{ fontSize: '12px', fontWeight: 900, color: '#0F172A' }}>{totalWindowDays} Days</span>
          </div>

          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: '3px solid #16A34A' }}>
            <span style={{ fontSize: '9px', fontWeight: 800, color: '#64748B', display: 'block', textTransform: 'uppercase' }}>Active Operational</span>
            <span style={{ fontSize: '12px', fontWeight: 900, color: '#16A34A' }}>{activeOperationalDays} Days</span>
          </div>

          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: '3px solid #D97706' }}>
            <span style={{ fontSize: '9px', fontWeight: 800, color: '#64748B', display: 'block', textTransform: 'uppercase' }}>Archive History</span>
            <span style={{ fontSize: '11px', fontWeight: 800, color: '#475569' }}>Archived {archiveCount}x | Paused {pausedDays}d</span>
          </div>

          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: '3px solid #8B5CF6' }}>
            <span style={{ fontSize: '9px', fontWeight: 800, color: '#64748B', display: 'block', textTransform: 'uppercase' }}>Target Requirement</span>
            <span style={{ fontSize: '12px', fontWeight: 900, color: '#0F172A' }}>{targetCount} {trackingMode === 'count_event' ? measureUnit : 'Days'}</span>
          </div>

          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: '3px solid #2563EB' }}>
            <span style={{ fontSize: '9px', fontWeight: 800, color: '#64748B', display: 'block', textTransform: 'uppercase' }}>Completed Progress</span>
            <span style={{ fontSize: '12px', fontWeight: 900, color: '#2563EB' }}>{currentCount} Completed</span>
          ...
          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: '3px solid #D97706' }}>
            ...
            <span style={{ fontSize: '12px', fontWeight: 900, color: '#D97706' }}>{remainingTargetCount} Left</span>
          </div>

          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: '3px solid #EC4899' }}>
            <span style={{ fontSize: '9px', fontWeight: 800, color: '#64748B', display: 'block', textTransform: 'uppercase' }}>Measure Unit</span>
            <span style={{ fontSize: '12px', fontWeight: 900, color: '#0F172A' }}>{currentTask.hasMeasureTracking ? `${measureTarget} ${measureUnit}/day` : 'Standard Completion'}</span>
          </div>

          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: '3px solid #6366F1' }}>
            <span style={{ fontSize: '9px', fontWeight: 800, color: '#64748B', display: 'block', textTransform: 'uppercase' }}>Repetition Frequency</span>
            <span style={{ fontSize: '12px', fontWeight: 900, color: '#0F172A' }}>{currentTask.recurrencePattern || 'Daily'}</span>
          </div>

          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: '3px solid #10B981' }}>
            <span style={{ fontSize: '9px', fontWeight: 800, color: '#64748B', display: 'block', textTransform: 'uppercase' }}>Parent Task</span>
            <span style={{ fontSize: '11px', fontWeight: 800, color: parentTask ? '#D97706' : '#64748B' }}>
              {parentTask ? parentTask.title : 'Root Task (None)'}
            </span>
          </div>

          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: '3px solid #F59E0B' }}>
            <span style={{ fontSize: '9px', fontWeight: 800, color: '#64748B', display: 'block', textTransform: 'uppercase' }}>Child Subtasks Count</span>
            <span style={{ fontSize: '12px', fontWeight: 900, color: '#0F172A' }}>{directChildSubtasks.length} Subtasks Mapped</span>
          </div>

          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: '3px solid #3B82F6' }}>
          ...
          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: isFeasible ? '3px solid #16A34A' : '3px solid #DC2626' }}>
            <span style={{ fontSize: '9px', fontWeight: 800, color: '#64748B', display: 'block', textTransform: 'uppercase' }}>Grace Days Gauge</span>
            <span style={{ fontSize: '12px', fontWeight: 900, color: isFeasible ? '#16A34A' : '#DC2626' }}>
          ...
          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: '3px solid #8B5CF6' }}>
            ...
            <span style={{ fontSize: '12px', fontWeight: 900, color: '#0F172A' }}>{requiredDailyPace} {measureUnit}/day</span>
          </div>

          <div style={{ background: '#F8FAFC', padding: '8px 10px', borderRadius: '8px', border: '1px solid #E2E8F0', borderLeft: parseFloat(paceDifference) >= 0 ? '3px solid #16A34A' : '3px solid #DC2626' }}>
            <span style={{ fontSize: '9px', fontWeight: 800, color: '#64748B', display: 'block', textTransform: 'uppercase' }}>Current Velocity Pace</span>
            <span style={{ fontSize: '12px', fontWeight: 900, color: parseFloat(paceDifference) >= 0 ? '#16A34A' : '#DC2626' }}>
      ...
      {(trackingMode === 'end_date' || trackingMode === 'count_days') && (
        <div className="glass-panel" style={{ padding: '16px', borderRadius: '14px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', flexWrap: 'wrap', gap: '8px' }}>
            <div>
              <h3 style={{ fontSize: '14px', fontWeight: 900, color: '#0F172A', margin: 0, display: 'flex', alignItems: 'center', gap: '6px' }}>
                <BarChart3 size={15} color="#DC2626" /> Daily Performance & Subtask Contribution Breakdown
              </h3>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
              {directChildSubtasks.slice(0, 4).map((st, i) => (
                <div key={st.id} style={{ display: 'flex', alignItems: 'center', gap: '3px', fontSize: '9px', fontWeight: 700, color: '#475569' }}>
          ...
          <div style={{ height: '180px', display: 'flex', alignItems: 'flex-end', gap: '6px', padding: '14px 6px 6px 6px', background: '#F8FAFC', borderRadius: '10px', border: '1px solid #E2E8F0' }}>
            {sampleDailyMeasures.map((d, i) => {
              return (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', height: '100%', justifyContent: 'flex-end' }}>
                  
                  {/* Stacked Subtask Column */}
                  <div style={{ width: '100%', maxWidth: '22px', display: 'flex', flexDirection: 'column-reverse', borderRadius: '4px', overflow: 'hidden', background: '#E2E8F0', height: `${Math.min(100, (d.totalVal / measureTarget) * 80)}%`, minHeight: '6px' }}>
                  ...
                  <span style={{ fontSize: '8px', fontWeight: 800, color: '#64748B', whiteSpace: 'nowrap' }}>
                    {d.dayLabel}
                  </span>
                </div>
              );
      ...
      {trackingMode === 'count_event' && (
        <div className="glass-panel" style={{ padding: '16px', borderRadius: '14px' }}>
          <h3 style={{ fontSize: '14px', fontWeight: 900, color: '#0F172A', margin: '0 0 2px 0', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <Activity size={15} color="#2563EB" /> Event Intensity Frequency Histogram
          </h3>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', background: '#F8FAFC', padding: '12px', borderRadius: '10px', border: '1px solid #E2E8F0' }}>
            {eventHistogramBins.map((bin, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span style={{ fontSize: '10px', fontWeight: 800, color: '#475569', width: '70px', flexShrink: 0 }}>
                  {bin.range}
                </span>

                <div style={{ flex: 1, height: '20px', background: '#E2E8F0', borderRadius: '4px', overflow: 'hidden', position: 'relative' }}>
                  <div 
                    style={{ 
                  ...
                  />
                  <span style={{ position: 'absolute', right: '6px', top: '2px', fontSize: '9px', fontWeight: 900, color: '#0F172A' }}>
                    {bin.count} sessions
                  </span>
                </div>
              </div>
      ...
      <div className="glass-panel" style={{ padding: '16px', borderRadius: '14px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', flexWrap: 'wrap', gap: '6px' }}>
          <div>
            <h3 style={{ fontSize: '14px', fontWeight: 900, color: '#0F172A', margin: 0, display: 'flex', alignItems: 'center', gap: '6px' }}>
              <Flame size={15} color="#DC2626" /> 365-Day Activity Matrix (LeetCode Grid)
            </h3>
            <p style={{ fontSize: '10px', color: '#64748B', margin: '2px 0 0 0' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '3px', fontSize: '9px', fontWeight: 800, color: '#64748B' }}>
            ...
            <div style={{ width: '8px', height: '8px', borderRadius: '2px', background: '#86EFAC' }} />
            <div style={{ width: '8px', height: '8px', borderRadius: '2px', background: '#4ADE80' }} />
            <div style={{ width: '8px', height: '8px', borderRadius: '2px', background: '#22C55E' }} />
            <div style={{ width: '8px', height: '8px', borderRadius: '2px', background: '#15803D' }} />
            <span>More</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '6px', overflowX: 'auto', paddingBottom: '4px' }}>
          {heatmapData.map((mObj, mIdx) => (
            <div key={mIdx} style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
              <span style={{ fontSize: '9px', fontWeight: 800, color: '#64748B', textAlign: 'center' }}>
                {mObj.month}
              </span>
      ...
      <div className="glass-panel" style={{ padding: '16px', borderRadius: '14px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', flexWrap: 'wrap', gap: '6px' }}>
          <div>
            <h3 style={{ fontSize: '14px', fontWeight: 900, color: '#0F172A', margin: 0, display: 'flex', alignItems: 'center', gap: '6px' }}>
              <CalendarDays size={15} color="#DC2626" /> Multi-View Calendar & Day Inspector
            </h3>
            <p style={{ fontSize: '10px', color: '#64748B', margin: '2px 0 0 0' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px', textAlign: 'center' }}>
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
            <div key={d} style={{ fontSize: '9px', fontWeight: 800, color: '#64748B', paddingBottom: '2px' }}>{d}</div>
          ))}
          ...
          {Array.from({ length: 28 }).map((_, idx) => {
            ...
            return (
              ...
              >
                <span style={{ fontSize: '11px', fontWeight: 800, color: isCompletedDay ? '#16A34A' : '#DC2626' }}>
                  {dayNum}
                </span>
        ...
        <div style={{ height: '140px', background: '#F8FAFC', padding: '12px', borderRadius: '10px', border: '1px solid #E2E8F0', position: 'relative', display: 'flex', alignItems: 'flex-end' }}>
          <svg style={{ width: '100%', height: '100%', overflow: 'visible' }}>
            ...
            <polyline 
              ...
              strokeWidth="2" 
              strokeDasharray="4,4" 
              points="0,110 70,85 140,60 210,40 280,20 350,5" 
            />
            ...
            <polyline 
              ...
              stroke="#2563EB" 
              strokeWidth="3" 
              points="0,110 70,95 140,65 210,35 280,20" 
            />
          </svg>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '12px' }}>
        ...
        <div className="glass-panel" style={{ padding: '16px', borderRadius: '14px', textAlign: 'center' }}>
          <h4 style={{ fontSize: '12px', fontWeight: 800, color: '#0F172A', margin: '0 0 8px 0' }}>
            Overall Goal Completion Gauge
          </h4>

          <div style={{ position: 'relative', width: '84px', height: '84px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <span style={{ position: 'absolute', fontSize: '16px', fontWeight: 900, color: '#0F172A' }}>
              {completionPercent}%
            </span>
          </div>
        ...
        <div className="glass-panel" style={{ padding: '16px', borderRadius: '14px' }}>
          <h4 style={{ fontSize: '12px', fontWeight: 800, color: '#0F172A', margin: '0 0 8px 0', display: 'flex', alignItems: 'center', gap: '4px' }}>
      ...
      <div className="glass-panel" style={{ padding: '16px', borderRadius: '14px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {filteredSubtasksList.length === 0 ? (
            <div style={{ fontSize: '11px', color: '#64748B', fontStyle: 'italic', padding: '8px', background: '#F8FAFC', borderRadius: '6px' }}>
            ...
            filteredSubtasksList.map(st => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <CornerDownRight size={12} color="#DC2626" />
                  <div>
                    <span style={{ fontSize: '12px', fontWeight: 800, color: '#0F172A' }}>{st.title}</span>
      ...
      {selectedCalendarDate && (
        <div style={{
          ...
          <div 
            ...
            style={{ width: '100%', maxWidth: '380px', padding: '16px', borderRadius: '14px', background: '#FFF' }}
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <div style={{ background: '#F0FDF4', border: '1px solid #BBF7D0', padding: '6px 10px', borderRadius: '6px', fontSize: '11px', fontWeight: 800, color: '#16A34A' }}>
                Task Turn Status: Completed
              </div>

              <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', padding: '6px 10px', borderRadius: '6px', fontSize: '11px', fontWeight: 800, color: '#0F172A' }}>
              ...
              {directChildSubtasks.slice(0, 2).map(s => (
                <div key={s.id} style={{ fontSize: '10px', color: '#16A34A', fontWeight: 700, background: '#DCFCE7', padding: '4px 6px', borderRadius: '5px' }}>
                  ✓ {s.title}
                </div>
              ))}
            </div>
